...
extsystem/                    연계시스템 관리 CRUD + 인증키 발급/재발급
  ...
  ExtSystemAdminMapper.java   CRUD + 매핑(API) 동기화 + updateCertKey + countApiDef + XML
...
- **null 파라미터**. `mybatis.configuration.jdbc-type-for-null=NULL` 설정됨(Oracle). 없으면 nullable insert 파라미터가 ORA-17004(부적합 열 유형 1111) 로 터진다. 새 insert 매퍼도 이 설정에 의존.
- **CLOB 컬럼**(ALW_IP_ADDR_TEXT/SQL_TEXT 등). String 파라미터로 그대로 insert 가능(ojdbc). JSON 컬럼은 `IS JSON` 제약 — 유효 JSON 문자열 전달.
...
| 레이트리밋(#4) | `gateway/RateLimiter` — 분당 한도(in-process, 전역 `app.gateway.rate-limit-per-min` 기본 600). **연계시스템별 override = `DXAPI_USR_EXT_SYS_M.RATE_LMT_PER_MIN`**(GatewayService 가 컬럼값 우선·NULL 이면 전역·0=무제한). 초과 429 RATE_LIMITED. 단일 인스턴스(다중=후속) | (#4a·#4b 완료) |
...
| GET | `/api/ext-systems` | ADMIN | ApiResponse(ItemsResponse) | extsystem |
| POST | `/api/ext-systems` | ADMIN | ApiResponse(extSystem+freshCertKey) | extsystem |
| GET | `/api/ext-systems/{id}` | ADMIN | ApiResponse(ExtSystemResponse) | extsystem |
