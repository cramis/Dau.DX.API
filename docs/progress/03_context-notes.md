...
## 2026-06-07 — 프로젝트 구조 문서 신설

### 한 일
- **`docs/01_프로젝트_구조.md` 신설** — 루트→앱별(backend/frontend/mcp) 디렉터리·패키지 지도 + 신규 코드 배치 규칙. "어디에 무엇이"만 다루고 구현 상태(✅/🟡)는 00 조망에 위임(역할 분리).
- README.md 3곳 연결 — 진입 순서 3번(00 다음), 라우팅 표, 생명주기 표 + 유지 규칙에 "구조 변경 시 01 동기화" 추가.
- `app/api/mock/` 함정(이름은 mock, 실은 다수 backendProxy)을 구조 문서에 ⚠ 명시.
