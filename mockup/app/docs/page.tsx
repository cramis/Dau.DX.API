// API 문서 뷰어 — 비로그인 접근 가능. 좌측 트리(그룹별) + 우측 상세(엔드포인트/파라미터/응답/curl).
"use client";
...
export default function Page() {
  ...
  useEffect(() => {
    void (async () => {
      const res = await fetch("/api/mock/apis");
      const data = await res.json().catch(() => ({}));
      if (data?.ok) {
        const visible = (data.items as ApiDef[]).filter((a) => a.docVisible);
        setApis(visible);
        if (visible.length > 0) setSelected(visible[0].no);
      }
      setLoaded(true);
    })();
  }, []);
  ...
  return (
    ...
    >
      ...
      >
        <Link href="/" className="w-brand" style={{ textDecoration: "none" }}>
      ...
      >
        ...
        >
          ...
          {loaded && groups.length === 0 && (
            <p className="w-muted" style={{ fontSize: 12, padding: 8 }}>
              검색 결과가 없습니다.
            </p>
          )}
          ...
          {!current ? (
            <div className="w-empty">
              <p className="w-empty__title">
                {loaded ? "왼쪽에서 API 를 선택하세요" : "불러오는 중…"}
              </p>
            </div>
          ) : (
